...
function toFormal(text, opts) {
  ...
  while (iTok < tokens.length) {
    ...
    if (t.type === "word") {
      ...
      if (opts?.decodeEmoji) {
        result.push(emoji_default[t.text] ?? t.text);
      } else {
        result.push(t.text);
      }
      iTok++;
...
function explainBreakdown(text) {
  ...
  for (let iTok = 0; iTok < tokens.length; iTok++) {
    ...
    if (t.type === "word") {
      ...
      const key = Object.keys(acronyms_default).find((k) => k.toLowerCase() === normalize(t.text));
      if (key) {
      ...
      const meaning = emoji_default[t.text];
...
// src/bg/service_worker.ts
var MENU_ID = "slang-tutor-translate-selection";
function log(...args) {
...
async function getSelectionFromPage(tabId) {
  try {
    ...
    const value = (res && res.result) ?? res;
...
async function handleTranslateClick(info, tab) {
